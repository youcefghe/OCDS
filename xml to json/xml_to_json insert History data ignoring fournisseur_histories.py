...
from table_creation import create_tables  # Ensure this creates your *_history tables*
...
# ----------------- Helper Functions (similar to your old script) -----------
def safe_str(val):
...
# ---------------------------------------------------------------------------
# 2) transform_bids_history -> from "avis_fournisseurs" only, ignoring fournisseurs_history
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3) transform_contrats_history -> contracts_history (SWAP logic)
# ---------------------------------------------------------------------------
...
def migrate_history_data():
    """
    1) Connect to old DB (XMLfinal) reading from *history tables* only:
       - avis_history
       - contrats_history
       - depenses_history
       (NOT fournisseurs_history, as requested).
    2) Connect to new DB (constructionDB) inserting into *history tables* only:
       - releases_history
       - contracts_history
       - contract_transactions_history
       - bids_history (from main 'avis_fournisseurs')
    3) We'll gather all ocids from the new DB's main 'releases' table.
    4) For each ocid, parse old DB's *history* (no category filter), plus 'avis_fournisseurs' for bids.
    5) No main table upserts; only direct inserts into *history tables*.
    6) Logging: we track how many remain for each ocid in the log.
    """
    try:
        ...
        # 2) For each ocid, parse old DB's *history*, ignoring fournisseurs_history,
        #    but we do produce bids_history from main 'avis_fournisseurs'.
